...
from projectbrain.ai.minimax import MiniMaxAdapter
...
class TestMiniMaxAdapterInit:
    ...
    def test_default_init(self):
        with patch("projectbrain.ai.minimax.env") as mock_env:
...
class TestMiniMaxEmbed:
    ...
    async def test_embed_single(self):
        ...
        with patch("projectbrain.ai.minimax.httpx.AsyncClient") as MockClient:
            mock_resp = MagicMock()
...
class TestMiniMaxAIAdapterInterface:
    ...
    def test_is_subclass_of_adapter(self):
        from projectbrain.ai.adapter import AIAdapter
...
class TestMiniMaxConfig:
    ...
    def test_config_has_minimax_fields(self):
        from projectbrain.core.config import EnvConfig
...
class TestMiniMaxExport:
    ...
    def test_exported_from_ai_package(self):
        from projectbrain.ai import MiniMaxAdapter as Imported
        assert Imported is MiniMaxAdapter

    def test_in_all_list(self):
        from projectbrain import ai
...
class TestEmbedDispatch:
    ...
    async def test_minimax_dispatch(self):
        with patch("projectbrain.memory.embed.MiniMaxAdapter") as MockAdapter:
            mock_instance = AsyncMock()
            mock_instance.embed = AsyncMock(return_value=[0.1] * 1536)
            MockAdapter.return_value = mock_instance

            with patch("projectbrain.memory.embed.env") as mock_env:
                mock_env.minimax_embedding_model = "embo-01"

                from projectbrain.memory.embed import emb_dispatch
