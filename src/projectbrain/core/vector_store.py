...
class SQLiteVectorStore(VectorStore):
    ...
    async def storeVector(self, id: str, sector: str, vector: List[float], dim: int, user_id: Optional[str] = None):
        ...
        db.execute(sql, (id, sector, user_id, blob, dim))
        db.commit()
...
def get_vector_store() -> VectorStore:
    backend = os.getenv("PROJECTBRAIN_VECTOR_STORE", "sqlite")

    if backend == "postgres":
        dsn = os.getenv("PROJECTBRAIN_PG_DSN", "postgresql://user:pass@localhost:5432/db")
        ...
        url = os.getenv("PROJECTBRAIN_REDIS_URL", "redis://localhost:6379/0")
