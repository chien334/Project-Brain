    ...
    for seg in segments:
        ...
        for m in batch:
            ...
            if f < max(0.3, cfg.cold_threshold):
                ...
                db.execute("UPDATE memories SET summary=? WHERE id=?", (fp["summary"], dict_m["id"]))
                tot_fp += 1
                changed = True

            if changed:
                db.execute("UPDATE memories SET salience=?, updated_at=? WHERE id=?", (new_sal, int(time.time()*1000), dict_m["id"]))
                tot_chg += 1
...
async def on_query_hit(mem_id: str, sector: str, reembed_fn = None):
    ...
    if cfg.reinforce_on_query:
        ...
        db.execute("UPDATE memories SET salience=?, last_seen_at=? WHERE id=?", (new_sal, int(time.time()*1000), mem_id))
        db.commit()
        updated = True
