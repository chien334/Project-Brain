...
async def batch_insert_facts(facts: List[Dict[str, Any]]) -> List[str]:
    ...
    try:
        ...
        for f in facts:
            ...
            meta = f.get("metadata")
            existing = db.fetchall("SELECT id, valid_from FROM temporal_facts WHERE subject=? AND predicate=? AND valid_to IS NULL", (sub, pred))
            for old in existing:
                 if old["valid_from"] < vf:
                     db.execute("UPDATE temporal_facts SET valid_to=? WHERE id=?", (vf - 1, old["id"]))
